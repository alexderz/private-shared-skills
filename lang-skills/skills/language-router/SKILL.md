...
- Process skills (`tdd`, `verify-before-done`, `pr-review`, `security-hardening`, `yagni`) may load with the one language skill.
...
- A second methodology router (`using-superpowers`, Pocock setup, Addy `using-agent-skills`).
- Remint `golang-*`, `modern-python`, or `shell-safety` under a new id.
- Language skills on README-only or git-only turns.
