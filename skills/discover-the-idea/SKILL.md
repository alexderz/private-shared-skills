...
attack. Compatible with `yagni`. Not a Superpowers / Pocock / Addy pack.
No `scripts/`. The subject does not have to be code.
...
- Install or invoke `mattpocock/skills` / `/grill-me` as a pack.
- Ask the user for something the environment already answers.
- Recommend a tool you have not looked at this session.
- Name pantheon personas. Roles here are architect / builder / tester /
